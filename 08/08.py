def part_one() -> None:
    ...
    for i in range(0, len(lines)):
        for j in range(0, len(lines[i])):
            ...
            if (
                any(int(num) >= int(lines[i][j]) for num in left)
                and any(int(num) >= int(lines[i][j]) for num in right)
                and any(int(num) >= int(lines[i][j]) for num in above)
                and any(int(num) >= int(lines[i][j]) for num in below)
            ):
                continue
            else:
                count += 1
